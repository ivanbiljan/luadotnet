...
public sealed class TypeMetadata
{
    ...
    /// </summary>
    public IEnumerable<MemberInfo> InstanceMembers => _instanceEvents.Cast<MemberInfo>()
        .Concat(_instanceFields)
        .Concat(_instanceMethods)
        .Concat(_instanceProperties);
    ...
    /// </summary>
    public IEnumerable<MemberInfo> StaticMembers => _staticEvents.Cast<MemberInfo>()
        .Concat(_staticFields)
        .Concat(_staticMethods)
        .Concat(_staticProperties);
    ...
    public static TypeMetadata Create(Type type)
    {
        ...
        var eventInfos = type.GetEvents(bindingFlags)
            .Where(e => e.GetCustomAttribute<LuaHideAttribute>() == null)
            .ToList();

        var fieldInfos = type.GetFields(bindingFlags)
            .Where(f => f.GetCustomAttribute<LuaHideAttribute>() == null)
            .ToList();

        var propertyInfos = type.GetProperties(bindingFlags)
            .Where(p => p.GetCustomAttribute<LuaHideAttribute>() == null)
            .ToList();
